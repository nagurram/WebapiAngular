...
export class UserService {
    ...
    userAuthentication(userName: string, password: string): Observable<any> {
        ...
        let headers = new HttpHeaders().set('Access-Control-Allow-Origin', '*').set('Content-Type', 'application/json').set('No-Auth', 'True').set('Access-Control-Allow-Methods', 'POST,GET,OPTIONS, PUT, DELETE').set('Access-Control-Allow-Headers', '*');
        let options = { headers: headers };
        return this._http.post(url, body, options);
